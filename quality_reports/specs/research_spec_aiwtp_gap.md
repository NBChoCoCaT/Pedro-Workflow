...
- **v7.1 (2026-06-01)**: Ch1 대상재 **lottery → 결정론적 산수 토큰** (methods·domain referee 검토: 복권은 induced value 아님, EV≠CE). nth-price IC·Lee positive control·인지능력 교란 동시 해결. Spatharioti(2023) 정답-있는-계산 과제 선례.
- **v7 (2026-05-31)**: ⭐ **전체 2-chapter 정합화 — induced/homegrown을 between-subject 2-chapter로 분리(ADR-011) + value knowability 통합 프레임(§2-6, §4-3).** 〔v7.2(2026-06-01): Qin capability–personalization 프레임 제거 — referee 검토(양적 전이·self-judgment 모순), 통합 축을 value knowability로 교체. Logg/Dietvorst는 배경 인용 유지.〕 Context·§1·§2(+신규 §2-6)·§3·§4·§5·§6(17편)·§7 모두 갱신. v6 자산(가상편의·건강식품·5-arm·Pre_Aversion·filler)은 Chapter 2에 보존.
...
- ADR-010: v2 → v6 pivot
- **ADR-011 (Amended 2026-06-01)**: induced 분리 실험 — **결정론적 산수 토큰**·AI 3-arm·인지능력 층화 (구 lottery → 토큰; referee 검토 반영)
...
> "We provide the first experimental test, in **incentive-compatible value elicitation**, of whether AI-provided information *helps or distorts* valuation — and show the effect's **character depends on whether the true value is knowable**. **Chapter 1** (induced-value **deterministic numeric token**, known answer) isolates pure behavioral distortion (anchoring / overreliance), since there is no true value to learn. **Chapter 2** (homegrown functional food, unknown value) measures hypothetical bias (Bias = WTP_CVM − WTP_auction): AI can genuinely inform *or* distort. Chapter 1 isolates the distortion channel and thereby informs Chapter 2 (cross-calibration). Cognitive ability moderates throughout. [v7.2: Qin framework removed; integrating axis = value knowability. v7.1: Ch1 = deterministic token, not lottery.]"

> **Framing:** Ch1 = Lee et al. (2020) induced-value 직접 확장(채널 B 순수 측정); Ch2 = Fox et al. (1998) CVM-X의 21세기 AI 재해석(가상편의). 두 챕터는 **value knowability(knowable vs unknowable)** 로 통합 (§4-3). gap: AI×유인적합 valuation 실증 부재(메타 163편 중 0편, lit-review·CoVe 확인).

---

## 1. 연구 질문 (Research Question)

> **중심 질문:** AI가 제공하는 정보는 incentive-compatible valuation을 *돕는가(정확한 가치 발견)* 아니면 *왜곡하는가(앵커링·과의존)*? 그리고 그 성격은 과제의 **value knowability(가치를 알 수 있느냐)**와 소비자 **인지능력**에 따라 어떻게 달라지는가?

**Chapter 1 — Induced Value (결정론적 산수 토큰, 정답 알려짐 → 순수 왜곡):**
...
**통합 (value knowability):**

- U-Q: AI 효과의 **성격이 Ch1(가치 knowable → 순수 왜곡)과 Ch2(가치 unknowable → 도움 또는 왜곡)에서 갈리는가**? 이는 인지능력으로 어떻게 조절되는가?

---

## 2. 이론적 프레임워크

> **v7 챕터 매핑:** §2-1(가상편의)·§2-2(채널 A, Bayesian)는 **Chapter 2 전용**(homegrown, 진짜 가치 불확실). **Chapter 1**(induced 결정론적 토큰)은 진짜 가치=정답값이 *고정*되어 채널 A(가치 발견)가 **부재** → §2-3(채널 B 왜곡)만 작동 = 순수 왜곡 측정. §2-3·§2-4는 양 챕터 공통. §2-6(value knowability)이 두 챕터를 통합하는 상위 프레임.
...
Logg et al. (2019)이 §Introduction(p.91)에서 Dietvorst et al. (2015)와 *직접 reconciliation*: 두 결과는 충돌이 아니라 *조건부 boundary* — Appreciation은 default(오류 관측 *전*), Aversion은 조건부(오류 관측 *후* + self-comparison).
...
**Chapter 1 — Induced (Deviation = |Bid − TrueValue|, 순수 채널 B):**
...
**통합 (value knowability):** Ch1(knowable) = AI 효과 *순수 왜곡* vs Ch2(unknowable) = AI 효과 *도움 또는 왜곡*. 양 챕터 모두 저인지능력에서 |효과| 증폭(탐색적).

### 2-6. 통합 프레임 — value knowability (진짜 가치를 알 수 있느냐)

두 챕터를 잇는 축은 *심리 태도*가 아니라 **설계가 직접 만드는 구분**: 진짜 가치를 알 수 있느냐.

- **Ch1 (산수 토큰, knowable):** 정답이 결정론적으로 알려짐 → AI는 *가치 발견을 도울 수 없고* 왜곡만 가능 → **채널 B 단독.**
- **Ch2 (건강식품, unknowable):** 진짜 가치를 아무도 모름(credence) → AI가 *진짜로 정보를 줄 수도(채널 A), 왜곡할 수도(채널 B).*
- → **AI 효과의 *성격*이 갈린다**(Ch1 순수 왜곡 vs Ch2 도움-or-왜곡). Ch1이 왜곡 채널을 분리 측정 → Ch2 해석을 보정(cross-calibration, §4-3).
- 배경: AI 신뢰는 맥락 의존적(Logg 2019 appreciation ↔ Dietvorst 2015 aversion) — 본 연구는 *유인적합 가치평가*에서 어느 쪽이 지배하는지를 value knowability·인지능력에 따라 본다. (유인적합 연구 0편 = gap.)

---

## 3. 실험 설계 — 2개 실험(2-chapter) 분리 구조

> **v7 개정(2026-05-31, ADR-011):** v6의 within-subject 2-phase 단일 실험을 **between-subject 2개 실험**으로 분리. 한 모집단에서 동시 수집하되 분석·집필은 **Chapter 1(induced 산수 토큰)** / **Chapter 2(homegrown 건강식품)**로 분리. 두 챕터는 value knowability(§4-3)로 통합. v6의 핵심 자산(가상편의·건강식품·5-arm·Pre_Aversion·filler)은 Chapter 2에 보존.
...
```
              ...
              ↓                                                          ↓
  ┌────────────────────────────────┐          ┌────────────────────────────────┐
  │ CHAPTER 1 — Induced (산수 토큰)  │          │ CHAPTER 2 — Homegrown (건강식품) │
  │ 진짜 가치 = 정답값 (결정론적)   │          │ 진짜 가치 불확실 (credence)       │
  │ AI 3-arm × 인지능력(RSPM)       │          │ AI 5-arm (v6 보존)               │
  │ 측정: Deviation = |Bid−TrueVal| │          │ 측정: Bias = WTP_CVM − WTP_auction│
  │ → AI 효과 = 순수 왜곡(채널 B)   │          │ → 가상편의 감소/왜곡(채널 A+B)    │
  └────────────────────────────────┘          └────────────────────────────────┘
              └── 통합: value knowability (Ch1 knowable=왜곡only, Ch2 unknowable=도움or왜곡) ──┘
```
...
### 3-2. Chapter 1 — Induced Value (결정론적 산수 토큰) 〔ADR-011 Amendment 2026-06-01〕

```
대상: 결정론적 산수 토큰 (예: 빨강 7×800 + 파랑 3×400 = 6,800원)
      위험·불확실성 0 → 정답값을 연구자가 정확히 앎; 계산은 필요(AI 보조 의미)
처치 (3-arm, between, RSPM 층 내 무작위):
  A0 통제 : AI 없음
  A1 정확 : AI가 올바른 토큰 정답값 제시
  A2 편향 : AI가 정답값 ±20% 제시 (방향 무작위 배정)
조절: 인지능력 RSPM 측정 → AI(3) × 능력(고/저 또는 연속) = 6셀
핵심 식별: 결정론적 정답 → Deviation = |Bid − TrueValue|가 순수 왜곡
          A2 ±20% 방향 무작위 = 앵커링 대칭성·단일방향 confound 통제
```
...
- **A2 − A1** = AI 정확도 민감도
- 정답값이 결정론적으로 고정이므로 모든 효과는 **베이지안 갱신이 아닌 순수 행동 왜곡(채널 B)** 으로 해석 (위험선호 교란 없음).
...
**Chapter 1 — 결정론적 산수 토큰 (induced value)** 〔ADR-011 Amendment〕**:**
- 상환가치 = 표시 수량의 **결정론적 산술 함수**(예: 7×800 + 3×400 = 6,800원). 위험 0 → 정답값 정확·연구자가 앎. *계산이 필요*하므로 AI 계산 보조 의미 유지; 산술 항 수로 인지부하 calibrate. 토큰 다중 라운드.
- AI 자극: "이 토큰의 정답값/추천 입찰: XX원" — **A1 = 정확 정답값**, **A2 = 정답값±20%(방향 무작위)**. 정답이 알려져 있어 `Bid−TrueValue`가 순수 왜곡.
- 선례: **Spatharioti(2023)** — "정답 있는 결정론적 계산 과제(cargo÷length ratio) + AI 정확/오류(rigged 4Runner)"와 동형.
...
**앵커값 설계(v6, 양 챕터 공통 ±20%):** Ch1 A2 = EV±20%; Ch2 T3/T4 = "AI 추정 적정 가치: XX원"(시장가 대비 ±20% 무작위), T2 = "시장 평균가: XX원"(generic ref). 방향 무작위화로 앵커링 방향성·대칭성 검증(Ch1은 토큰이라 위험선호 교란 자체가 없음). 생태적 타당성 한계는 Limitation에 명시.
...
- **결정론적 토큰 good** → nth-price = Vickrey 유인적합성 성립. (구 lottery 설계의 BDM·non-EU 쟁점은 토큰 전환으로 소멸.)
...
2. (선택) 사전 산술 정확도 — A1 계산보조 baseline 〔위험선호(Holt–Laury)는 토큰 전환으로 불필요〕
...
```
[공통 사전] 동의 → 사전설문(RSPM, Pre_Aversion, (Ch2)건강프로필) → 메커니즘 교육·연습

[CHAPTER 1]  처치(A0/A1/A2) → 토큰 경매(다중 라운드·no-feedback)
             측정: Deviation = |Bid − TrueValue|, AnchorDev = Bid − AI_value (A2)
...
- **Chapter 2:** 5-arm — v6 기준(Choi 2018 ±15% base rate, Fox CVM-X 효과크기, Bias 표준편차 기반 집단 간 검정). 구매경험은 연속 통제로 표본 영향 없음.
...
> 〔ADR-011 Amendment 2026-06-01: 대상재 = 결정론적 산수 토큰. 정답값이 결정론적이라 위험선호 교란 없음 → CE≠EV 문제 소멸, A1/A2 해석 명료.〕

```
Deviation_i = |Bid_i − TrueValue_i|   (정답값 결정론적 고정 → AI 효과 = 순수 행동 왜곡)
...
(2) 앵커링 식별 (A2):
  Bid_i = α + λ·(AI_value_i − TrueValue_i) + (controls) + ε_i
  AI_value = TrueValue·(1 ± 0.20), 방향 무작위 → λ > 0 = 앵커링 강도
  ±20% 방향 무작위는 앵커링 대칭성·단일방향 confound 통제
  (결정론적 토큰이라 위험선호 교란 자체가 없음 → 구 "CE 구성·Holt–Laury" 불필요)
...
### 4-3. 통합 분석 — value knowability [v7]

```
프레임: AI 효과는 진짜 가치를 알 수 있느냐에 따라 성격이 갈린다.
  Ch1 (knowable, 토큰)    → AI는 발견 불가 → 순수 왜곡(채널 B)
  Ch2 (unknowable, credence) → AI 정보 제공(A) 또는 왜곡(B)

검정:
  - 챕터 × AI 상호작용으로 Ch1(왜곡-only)과 Ch2(도움-or-왜곡) 효과 성격 대비
  - Ch1의 채널 B 추정치로 Ch2의 채널 A/B 해석 보정 (cross-calibration)

주의: cross-calibration은 두 재화·표본 차이 때문에 *정량* 보정이 아니라
      "채널 B가 유의미하게 존재함"의 *정성적 floor*로 한정(referee MAJOR-4 반영).
```
...
2단계 Structured Secondary:
  Ch2 SUR 비대칭(β_auction vs β_CVM); 챕터×AI 효과차이(value knowability); mediation → nested model
...
**Ch1의 단순화:** 정답값 결정론적 고정이라 채널 A(베이지안 가치발견)가 부재 → induced에선 **채널 B 단독 식별** → mediation이 단순. 즉 Ch1이 Ch2의 채널 A/B 등가성 문제를 *외부에서 보정*(cross-calibration).
...
1. Introduction
   - Hook: AI가 의사결정을 개선한다는 통념(Logg 2019 appreciation) vs.
           Corrigan 2012(정보가 demand revelation 악화) + Dietvorst 2015(aversion)
   - Puzzle: AI는 가치 발견을 돕나, 앵커로 왜곡하나? 방향은 과제에 따라 뒤집히나?
   - Research Q: 유인적합 valuation에서 AI 효과 + value knowability·인지능력 조절
   - Preview: 2-chapter (Ch1 induced 산수 토큰 / Ch2 homegrown 건강식품) + value knowability 통합
   - Framing: Lee 2020(Ch1)·Fox CVM-X(Ch2); 통합 축 = value knowability
...
5. Expected Contributions & Limitations
   ...
   - 한계: 외적 타당성 / Bergman CRT×앵커링 비유의 / AI 금전앵커 생태타당성 /
           cross-sample 보정의 "동일 AI 효과" 가정(→ 정성적 floor로 한정)
...
| **Spatharioti et al. (2023) arXiv:2307.03744** | 정답-있는 결정론적 계산 과제 + LLM 정확→무해/오류→과의존(47% vs 93%) — **Ch1 토큰·A1/A2 선례**(preprint) |
...
| "Ch1 이탈이 위험선호 때문 아닌가?" | **결정론적 산수 토큰**이라 위험·불확실성이 없음 → 정답 단일·명확, 이탈=순수 왜곡(구 lottery의 CE≠EV 문제 소멸, ADR-011 Amendment). A2 ±20% 방향 무작위는 앵커링 대칭성 통제 |
...
| "두 챕터를 왜 묶나?" | **value knowability** — Ch1(가치 knowable=토큰)은 AI가 왜곡만 가능, Ch2(unknowable=credence)는 도움 또는 왜곡. Ch1이 왜곡 채널을 분리 측정해 Ch2 해석을 보정(cross-calibration, 정성적 floor) |
| "Logg/Dietvorst 충돌 아닌가?" | Logg 본인 reconciliation(appreciation=default, 오류 후 aversion). 본 설계는 *유인적합 가치평가*에서 어느 쪽이 지배하는지 검정 |
...
| "채널 A와 B-1이 등가하지 않나?" (Ch2) | 완전 분리 불가 인정 → Generic AnchorDev로 간접 분리 + mediation; **Ch1(정답값 결정론적 고정)이 채널 B를 외부 식별해 Ch2를 보정**(cross-calibration) |
...
특히 Logg-Dietvorst boundary condition 정합은 본 연구 **between 분리 + Pre_Aversion** 디자인의 *학문적 정당화*를 강화함 — reviewer가 "Algorithm Appreciation/Aversion이 충돌하지 않나?" 질문 시 *Logg 본인의 reconciliation*(appreciation=default, 오류 후 aversion)을 인용하여 직접 대응 가능.
